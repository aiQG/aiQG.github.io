...
运行 [iReSign](https://github.com/maciekish/iReSign) 项目, 分别填上要签名的.ipa,  `.mobileprovision` 文件路径, (.plist不用填), 项目的Bundle Identifier(可以在项目的配置文件里找到), 证书(和项目的Bundle Identifier 对应, 同样可以在项目的配置文件里找到)

// 注意⚠️: 选择.plist路径的位置应该留空(程序会自动根据.mobileprovision文件导出.plist). 除非有特殊需要
